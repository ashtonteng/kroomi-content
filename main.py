...
def youtube_url_to_json_local(
    ...
    transcript_with_timestamps = fs.get_saved_file_if_exists(fs.TRANSCRIPTS_DIR, video_id)
    if not transcript_with_timestamps:
        ...
        fs.write_saved_file(fs.TRANSCRIPTS_DIR, video_id, transcript_with_timestamps)
        rewrite = True

    transcript_without_timestamps = fs.get_saved_file_if_exists(fs.TRANSCRIPTS_NO_TIMESTAMPS_DIR, video_id)
    if not transcript_without_timestamps:
        ...
        fs.write_saved_file(fs.TRANSCRIPTS_NO_TIMESTAMPS_DIR, video_id, transcript_without_timestamps)
        rewrite = True

    if not extractor:
        return

    extracted_protocol = fs.get_saved_file_if_exists(fs.EXTRACTED_NO_TIMESTAMPS_DIR, video_id)
    ...
    if not extracted_protocol or rewrite:
        ...
        fs.write_saved_file(fs.EXTRACTED_NO_TIMESTAMPS_DIR, video_id, extracted_protocol)
        rewrite = True

    if not timestamper:
        return

    timestamped_protocol = fs.get_saved_file_if_exists(fs.TIMESTAMPS_ADDED_DIR, video_id)
    if not timestamped_protocol or rewrite:
        ...
        fs.write_saved_file(fs.TIMESTAMPS_ADDED_DIR, video_id, timestamped_protocol)

    if not formatter:
        return

    formatted_protocol = fs.get_saved_file_if_exists(fs.FORMATTED_DIR, video_id)
    if not formatted_protocol or rewrite:
        ...
        fs.write_saved_file(fs.FORMATTED_DIR, video_id, formatted_protocol)
        rewrite = True

    if not output_json:
        return

    formatted_json = fs.get_saved_file_if_exists(fs.FINAL_JSON_DIR, video_id)
    if not formatted_json or rewrite:
        ...
        fs.write_saved_file(fs.FINAL_JSON_DIR, video_id, formatted_json)
